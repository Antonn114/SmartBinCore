...
class SBStepperMotor(DRV8825):
    def __init__(self, dir_pin, step_pin, enable_pin, mode_pins):
        super().__init__(dir_pin, step_pin, enable_pin, mode_pins)
        self.SetMicroStep('softward','fullstep')
    
    def go_CCW(self, power):
        self.TurnStep('forward', power, stepdelay=0.001)
    
    def go_CW(self, power):
        self.TurnStep('backward', power, stepdelay=0.001)
    
    def go_and_return(self, power, callback):
        self.go_CCW(power)
        if (callback != None):
            callback
        self.go_CW(power)
